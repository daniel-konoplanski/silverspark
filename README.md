...
**system-wide** so an Administrator PowerShell can find it on PATH:

```
winget install astral-sh.uv
```

Then create the project environment from the lockfile:

```
uv sync
```

This creates a local `.venv/` with the exact locked dependency versions.
(If you ever need a plain pip list, `uv export --format requirements-txt` regenerates one.)
...
```
uv run silverspark.py --calibrate
```
...
- `silverspark.py` — the bot (the `CONFIG` block is at the top, then the workflow).
- `pyproject.toml` — project metadata + dependencies. **This is the file you edit** to
  change dependencies (`uv add` / `uv remove` do it for you).
- `uv.lock` — exact locked versions, auto-generated by uv and committed for reproducible
  installs. Don't hand-edit it; let uv regenerate it.
- `pictures/` — reference screenshots used to derive the coordinates (gitignored; local only).
